...
class LoanController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('sacco/loans/Index', [
            ...
            'quarters' => $quarters,
            'statuses' => $statuses,
            'allActiveDisbursedLoans' => $allActiveDisbursedLoans,
        ]);
    }
    ...
    /**
     * Record repayments for selected months on a single loan.
     */
    public function batchRepayForLoan(Request $request, string $loanId, LoanRepaymentService $service)
    {
        $user = Auth::user();
        if (!$user->isAdmin()) {
            abort(403);
        ...
        if ($loan->status !== 'disbursed') {
            return response()->json(['message' => 'Can only record repayments for disbursed loans.'], 422);
        }

        if ($loan->outstanding_balance <= 0) {
            return response()->json(['message' => 'Loan is already fully repaid.'], 422);
        }

        $request->validate([
            'selected_months' => ['required', 'array', 'min:1'],
            'selected_months.*' => ['string', 'regex:/^\d{4}-\d{2}$/'],
            'dry_run' => ['nullable', 'boolean'],
        ]);

        $dryRun = $request->boolean('dry_run', true);
        $selectedMonths = array_values(array_unique($request->input('selected_months', [])));

        try {
            $summary = $service->createRepaymentsForLoan($loan, $selectedMonths, $dryRun, $user->id, true);
